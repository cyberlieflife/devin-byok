...
func scheduleApply(extractDir, installDir, guiName, tmp string) (string, error) {
	...
	ps := strings.Join([]string{
		...
		"$src = " + srcPS,
		"$dst = " + dstPS,
		"$gui = Join-Path $dst " + powershellSingleQuote(guiName),
		"$srcGui = Join-Path $src " + powershellSingleQuote(guiName),
		...
		"  }",
		"}",
		"if (-not $writable) { Write-Host '[devin-byok] ERROR: target still locked after 40s'; exit 1 }",
		"Write-Host '[devin-byok] applying update to' $dst",
		"New-Item -ItemType Directory -Force -Path $dst | Out-Null",
		"Copy-Item -LiteralPath $srcGui -Destination $gui -Force",
		"if (-not (Test-Path -LiteralPath $gui)) { Write-Host '[devin-byok] ERROR: copy failed'; exit 1 }",
