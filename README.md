...
1. 解压 zip  
2. 运行 `./devin-byok-gui`（首次需要 Allow 安全设置）  
...
# dist/devin-byok-<ver>-darwin-arm64.zip
```
...
# 上传各平台 zip + sha256 到 GitHub Release
gh release create v1.2.0 dist/devin-byok-1.2.0-darwin-arm64.zip dist/devin-byok-1.2.0-darwin-arm64.zip.sha256 dist/devin-byok-1.2.0-darwin-x64.zip dist/devin-byok-1.2.0-darwin-x64.zip.sha256 dist/devin-byok-1.2.0-windows-amd64.zip dist/devin-byok-1.2.0-windows-amd64.zip.sha256 --title v1.2.0
```
...
### macOS
发布包只需 `devin-byok-gui`（+ `START.txt`）：内嵌 local-api、默认配置模板、LS wrapper。
