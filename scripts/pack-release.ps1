...
if (-not $Version) {
  $vg = Get-Content ".internalersionersion.go" -Raw
...
New-Item -ItemType Directory -Force -Path .internalpayload | Out-Null
if (Test-Path .config.example.yaml) {
  Copy-Item .config.example.yaml .internalpayloadconfig.example.yaml -Force
} elseif (-not (Test-Path .internalpayloadconfig.example.yaml)) {
  throw "missing config.example.yaml for embed payload"
}
go build -ldflags "-s -w" -o .internalpayloadls-wrapper.exe ./cmd/ls-wrapper
...
$rsrc = Join-Path (go env GOPATH) "binsrc.exe"
...
if (Test-Path $rsrc) {
  & $rsrc -ico ".internaldesktopicon.ico" -arch amd64 -o ".cmddevin-byok-guisrc_windows_amd64.syso"
